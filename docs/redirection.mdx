...
### Route-level redirection considerations
...
GoRoute(
  path: '/',
  redirect: (_) => '/elsewhere', // only called when going to /
  pageBuilder: (context, state) => ..., // need this to build /somewhere stack
  routes: [
    GoRoute(
      path: 'somewhere',
      pageBuilder: ...,
...
stack of matched routes.

Of course, in you don't have a route-level `redirect` function at all, you will
need a `pageBuilder` function in that case as well.
