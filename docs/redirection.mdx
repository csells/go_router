...
screen. That's because the go_router doesn't know that the app's state has
...
certain conditions, you will still need to provide a `pageBuilder` function,
e.g.
...
GoRoute(
  ...
  pageBuilder: (context, state) => ..., // need this if kGoElsewhere == false
)
```

The `pageBuilder` function will be called when the redirect condition is false.

Also, if the `redirect` function is part of a sub-route, you will need to
provide a `pageBuilder` function, e.g.

```dart
GoRoute(
  path: '/profile',
  redirect: (_) => '/profile/home', // only called when going to /profile
  pageBuilder: (c, s) => ..., // need this to build /profile/:section stack
  routes: [
    GoRoute(
      path: ':section',
      pageBuilder: ...ProfilePage(state.params['section']!)...,
    ),
  ],
)
```
...
stack of matched routes, so if the `redirect` is provided on a route in the middle
of the stack, the `pageBuilder` will be called when building pages for the stack
above.

In fact, you probably don't want to have a `redirect` in the middle of your stack
of pages anyway. You probably want the `redirect` function on a stack all itself,
redirecting to another top-level route:

```dart
GoRoute(
  path: '/profile',
  redirect: (_) => '/profile/home', // pageBuilder not needed in this case
),
GoRoute
...
route, you will need a `pageBuilder` function in that case as well.
