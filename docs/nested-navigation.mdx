...
Of course, you can easily do this using the `TabBarView` widget, but what makes
...
that screen with the `TabBarView` above, you need a widget that changes the
...
alone, to update the browser's address bar and to let the `TabBarView` navigate to
the new selection.
...
This example shows off the selected tab on a `TabBarView` but you can use it for
...
action in the `FamilyView` of the
