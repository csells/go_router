...
Furthermore, assuming you wanted to let go_router pick the `key` and
`restorationId` for each of your pages instead of managing them yourself, you
still had to manually pass those parameters to each `MaterialPage` instance (or
`CupertinoPage` instance).
...
the `pageBuilder` that you expect. You can continue to use the `pageBuilder` if
you want to specify the key or some other parameter to the page-type you're
creating:
